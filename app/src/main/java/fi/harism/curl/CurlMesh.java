...
public class CurlMesh {
	...
	public static final String TAG = CurlMesh.class.getSimpleName();
	...
	private final int HARD_VERTICES_COUNT = 4;
	private final int HARD_SHADOW_SPLITS = 3;
	...
	private FloatBuffer mVerticesHard;
	private FloatBuffer mTexCoordsHard;
	private FloatBuffer mColorsHard;

	private int mVerticesCountFront;
	private int mVerticesCountBack;

	private int mVerticesCountFrontHard;
	private int mVerticesCountBackHard;

	private FloatBuffer mShadowColors;
	private FloatBuffer mShadowColorsHard;
	private FloatBuffer mShadowVertices;
	private FloatBuffer mShadowVerticesHard;
	private int mDropShadowCount;
	private int mSelfShadowCount;
	private int mDropShadowCountHard;
	private int mSelfShadowCountHard;

	private CurlPageProvider mCurlPageProvider;
	...
	private Array<Vertex> mHardVertices;
	private Array<Vertex> mTempHardVertices;

	private Array<ShadowVertex> mTempShadowVerticesHard;
	private Array<ShadowVertex> mSelfShadowVerticesHard;
	private Array<ShadowVertex> mDropShadowVerticesHard;
	...
	public CurlMesh(int maxCurlSplits) {
		...
		mHardVertices = new Array<>( HARD_VERTICES_COUNT );
		mTempHardVertices = new Array<>( HARD_VERTICES_COUNT );
		for (int i = 0; i < HARD_VERTICES_COUNT; ++i )
			mTempHardVertices.add(new Vertex());
		...
		if (DRAW_SHADOW) {
			...
			mSelfShadowVerticesHard = new Array<>( HARD_VERTICES_COUNT * HARD_SHADOW_SPLITS );
			mDropShadowVerticesHard = new Array<>( HARD_VERTICES_COUNT * HARD_SHADOW_SPLITS );

			mTempShadowVerticesHard = new Array<>( HARD_VERTICES_COUNT * HARD_SHADOW_SPLITS * 2 );
			for (int i = 0; i <  HARD_VERTICES_COUNT * HARD_SHADOW_SPLITS * 2; ++i)
				mTempShadowVerticesHard.add( new ShadowVertex() );
		}
		...
		// For hard page, there are 4 vertices
		final int verticesHardCount = 4;

		vbb = ByteBuffer.allocateDirect(verticesHardCount * 3 * 4);
		vbb.order(ByteOrder.nativeOrder());
		mVerticesHard = vbb.asFloatBuffer();
		mVerticesHard.position(0);
		...
		if (DRAW_TEXTURE) {
			...
			tbb = ByteBuffer.allocateDirect( verticesHardCount * 2 * 4 );
			tbb.order(ByteOrder.nativeOrder());
			mTexCoordsHard = tbb.asFloatBuffer();
			mTexCoordsHard.position(0);
		}
		...
		cbb = ByteBuffer.allocateDirect(verticesHardCount * 4 * 4);
		cbb.order(ByteOrder.nativeOrder());
		mColorsHard = cbb.asFloatBuffer();
		mColorsHard.position();
		...
		if (DRAW_SHADOW) {
			...
			final int maxShadowVerticesHardCount = verticesHardCount * 2 * 4;
			scbb = ByteBuffer.allocateDirect( maxShadowVerticesHardCount * 4 * 4);
			scbb.order(ByteOrder.nativeOrder());
			mShadowColorsHard = scbb.asFloatBuffer();
			mShadowColorsHard.position(0);

			sibb = ByteBuffer.allocateDirect(maxShadowVerticesHardCount * 3 * 4);
			sibb.order(ByteOrder.nativeOrder());
			mShadowVerticesHard = sibb.asFloatBuffer();
			mShadowVerticesHard.position(0);

			mDropShadowCountHard = mSelfShadowCountHard = 0;
		}
	}

	public void setCurlPageProvider(CurlPageProvider curlPageProvider) {

		mCurlPageProvider = curlPageProvider;
	}
	...
	public void onDetach(){
		...
		if( mBitmapB != null && mBitmapB.isRecycled() == false ) {
	...
	public synchronized void curl(PointF curlPos, PointF curlDir, double radius) {
		...
		mVerticesCountFrontHard = mVerticesCountBackHard = 0;

		mVerticesCountFrontHard = 4;

		final float totalWidth = Math.max( Math.abs( (float)mRectangle[1].mPosX ),(float)mRectangle[3].mPosX );

		Vertex vertexHard;
		mTempHardVertices.addAll( mHardVertices );
		mHardVertices.clear();
		double hardMaxZPos = 0;
		for ( int i = 0; i < HARD_VERTICES_COUNT; ++i ) {

			vertexHard = mTempHardVertices.remove( 0 );
			vertexHard.set( mRectangle[i] );

			// Direction right-to-left and page has passed middle to the left | |<|  |
			// moving vertices i > 1
			if( curlDir.x < 0.0f && i > 1 ){

				vertexHard.mPosX = ( curlPos.x * 2f ) - totalWidth;// + 0.2f;
				vertexHard.mPosY += ( totalWidth - Math.abs( vertexHard.mPosX ) ) * 0.1f;
				if( vertexHard.mPosX < 0.f )
					mVerticesCountBackHard ++;
			}
			// Direction left-to-right and page has passed middle to the right  |  |>| |
			// moving vertices i <= 1
			else if( curlDir.x >= 0.0f && i <= 1 ) {

				vertexHard.mPosX = (curlPos.x * 2) + totalWidth ;//- 0.2f;
				vertexHard.mPosY += ( totalWidth - Math.abs( vertexHard.mPosX ) ) * 0.1f;
			...
			if( Math.abs( vertexHard.mPosX ) > 0 )
				vertexHard.mPosZ = Math.sqrt( Math.pow( totalWidth /2 , 2 )
						- Math.pow( (Math.abs( vertexHard.mPosX  ) /*- 0.2f*/ ) /2 , 2 ) );

			hardMaxZPos = Math.max( hardMaxZPos, vertexHard.mPosZ );
			mHardVertices.add( i, vertexHard  );
			addHardVertex( vertexHard );
		}
		...
		if (DRAW_SHADOW) {
			...
			mTempShadowVerticesHard.addAll( mSelfShadowVerticesHard );
			mTempShadowVerticesHard.addAll( mDropShadowVerticesHard );
			mSelfShadowVerticesHard.clear();
			mDropShadowVerticesHard.clear();
		}

		if( DRAW_SHADOW && hardMaxZPos > 0.01f ){

			ShadowVertex selfShadowHardVertex;
			ShadowVertex dropShadowHardVertex;
			int idx = 0;
			for ( int hardVertexIndex = 0; hardVertexIndex  < HARD_VERTICES_COUNT; ++hardVertexIndex ) {

				vertexHard = mHardVertices.get( hardVertexIndex );

				selfShadowHardVertex = mTempShadowVerticesHard.remove( 0 );
				selfShadowHardVertex.mPosX = vertexHard.mPosX;
				selfShadowHardVertex.mPosY = vertexHard.mPosY;
				selfShadowHardVertex.mPosZ = vertexHard.mPosZ;
				//selfShadowHardVertex.mPenumbraX = ( ( vertexHard.mPosZ - radius ) / 3 ) * vertexHard.mPenumbraX;
				selfShadowHardVertex.mPenumbraX = vertexHard.mPenumbraX / 30 * (mVerticesCountBackHard >0 ? -1 : 1);
				//selfShadowHardVertex.mPenumbraY = ( ( vertexHard.mPosZ - radius ) / 3 ) * vertexHard.mPenumbraY;
				selfShadowHardVertex.mPenumbraY = vertexHard.mPenumbraY / 30;
				selfShadowHardVertex.mPenumbraColor = ( vertexHard.mPosZ/2 - radius ) / ( 2 * radius );
				idx = ( mSelfShadowVerticesHard.size() + 1 ) / 2;
				mSelfShadowVerticesHard.add( idx, selfShadowHardVertex );

				// static vertices
				if ( ( curlDir.x < 0.0f && hardVertexIndex <= 1 ) || ( curlDir.x >= 0.0f && hardVertexIndex > 1 ) ) {

					dropShadowHardVertex = mTempShadowVerticesHard.remove( 0 );

					dropShadowHardVertex.mPosX = vertexHard.mPosX;
					dropShadowHardVertex.mPosY = vertexHard.mPosY;
					dropShadowHardVertex.mPosZ = hardMaxZPos;
					dropShadowHardVertex.mPenumbraX = ( hardMaxZPos/ 6 ) * -curlDir.x *( mVerticesCountBackHard > 0 ? -1 : 1 );
					dropShadowHardVertex.mPenumbraY = (vertexHard.mPosY / 2) * ( hardMaxZPos / 6 );
					dropShadowHardVertex.mPenumbraColor = hardMaxZPos / (radius * 6 );
					idx = ( mDropShadowVerticesHard.size() + 1 ) / 2;
					mDropShadowVerticesHard.add( idx, dropShadowHardVertex );
				}
			}
		}
		...
		if (DRAW_SHADOW) {
			...
			mDropShadowCount = 0;
			mSelfShadowCount = 0;
			mDropShadowCountHard = 0;
			mSelfShadowCountHard = 0;

			for (int i = 0; i < mDropShadowVertices.size(); ++i) {

				addShadowVertex( mDropShadowVertices.get(i), mShadowVertices, mShadowColors );
				mDropShadowCount += 2;
			}

			for (int i = 0; i < mSelfShadowVertices.size(); ++i) {

				addShadowVertex( mSelfShadowVertices.get(i), mShadowVertices, mShadowColors );
				mSelfShadowCount += 2;
			}

			for (int i = 0; i < mDropShadowVerticesHard.size(); ++i){

				addShadowVertex( mDropShadowVerticesHard.get(i), mShadowVerticesHard, mShadowColorsHard );
				mDropShadowCountHard += 2;
			}

			for (int i = 0; i < mSelfShadowVerticesHard.size(); ++i){

				addShadowVertex( mSelfShadowVerticesHard.get(i), mShadowVerticesHard, mShadowColorsHard );
				mSelfShadowCountHard += 2;
			}
	...
	private void addShadowVertex( final ShadowVertex shadowVertex,
								  FloatBuffer shadowVertices,
								  FloatBuffer shadowColors  ){

		shadowVertices.put( ( float ) shadowVertex.mPosX );
		shadowVertices.put( ( float ) shadowVertex.mPosY );
		shadowVertices.put( ( float ) shadowVertex.mPosZ );
		shadowVertices.put( ( float ) ( shadowVertex.mPosX + shadowVertex.mPenumbraX ) );
		shadowVertices.put( ( float ) ( shadowVertex.mPosY + shadowVertex.mPenumbraY ) );
		shadowVertices.put( ( float ) shadowVertex.mPosZ );

		for ( int j = 0; j < 4; ++j ) {
			double color = SHADOW_OUTER_COLOR[ j ]
					+ ( SHADOW_INNER_COLOR[ j ] - SHADOW_OUTER_COLOR[ j ] )
					* shadowVertex.mPenumbraColor;
			shadowColors.put( ( float ) color );
		}

		shadowColors.put( SHADOW_OUTER_COLOR );
	}
	...
	public synchronized void drawHard(GL10 gl){

		// 1. First allocate texture if there is not one yet.
		...
		// 2. If mBitmap != null we have a new texture.
		...
		if ( DRAW_TEXTURE ) {
			...
			gl.glTexCoordPointer( 2, GL10.GL_FLOAT, 0, mTexCoordsHard );
		}

		gl.glVertexPointer( 3, GL10.GL_FLOAT, 0, mVerticesHard );

		//6. Enable color array.
		gl.glEnableClientState( GL10.GL_COLOR_ARRAY );
		gl.glColorPointer( 4, GL10.GL_FLOAT, 0, mColorsHard );

		// 7. Draw blank / 'white' front facing vertices.
		gl.glDisable( GL10.GL_TEXTURE_2D );
		gl.glDrawArrays( GL10.GL_TRIANGLE_STRIP, 0, mVerticesCountFrontHard );
		...
		if ( mVerticesCountBackHard > 0 ) {
			// 8. Draw blank / 'white' back facing vertices.
			gl.glDisable( GL10.GL_TEXTURE_2D );
			gl.glDrawArrays( GL10.GL_TRIANGLE_STRIP, 0, mVerticesCountFrontHard );
		...
		// 9. Disable textures and color array.
		...
		if ( DRAW_POLYGON_OUTLINES ) {
			...
			gl.glVertexPointer( 3, GL10.GL_FLOAT, 0, mVerticesHard );
			gl.glDrawArrays( GL10.GL_LINE_STRIP, 0, mVerticesCountFrontHard );
			gl.glDisable( GL10.GL_BLEND );
		}

		// 4. Drop Shadow
		// problems with its calculation.
		if ( DRAW_SHADOW && mDropShadowCountHard > 0 ) {
			gl.glEnable( GL10.GL_BLEND );
			gl.glBlendFunc( GL10.GL_SRC_ALPHA, GL10.GL_ONE_MINUS_SRC_ALPHA );
			gl.glEnableClientState( GL10.GL_COLOR_ARRAY );
			gl.glColorPointer( 4, GL10.GL_FLOAT, 0, mShadowColorsHard );
			gl.glVertexPointer( 3, GL10.GL_FLOAT, 0, mShadowVerticesHard );
			gl.glDrawArrays( GL10.GL_TRIANGLE_STRIP, 0, mDropShadowCountHard );
			gl.glDisableClientState( GL10.GL_COLOR_ARRAY );
			gl.glDisable( GL10.GL_BLEND );
		}

		// 10. Self Shadow
		if ( DRAW_SHADOW && mSelfShadowCountHard > 0) {
			gl.glEnable( GL10.GL_BLEND );
			gl.glBlendFunc( GL10.GL_SRC_ALPHA, GL10.GL_ONE_MINUS_SRC_ALPHA );
			gl.glEnableClientState( GL10.GL_COLOR_ARRAY );
			gl.glColorPointer( 4, GL10.GL_FLOAT, 0, mShadowColorsHard );
			gl.glVertexPointer( 3, GL10.GL_FLOAT, 0, mShadowVerticesHard );
			gl.glDrawArrays( GL10.GL_TRIANGLE_STRIP, mDropShadowCountHard, mSelfShadowCountHard );
			gl.glDisableClientState( GL10.GL_COLOR_ARRAY );
			gl.glDisable( GL10.GL_BLEND );
		}
	...
	public synchronized void drawCurled(GL10 gl){
		// 1. First allocate texture if there is not one yet.
		if (DRAW_TEXTURE && mTextureIds == null) {
			// Generate texture.
			mTextureIds = new int[2];
	 ...
	 */
	public synchronized void setBitmap(Bitmap bitmap, Bitmap bitmapB, final boolean isHard ) {
		...
		if (DRAW_TEXTURE) {
			...
			if(mBitmap != null)
				mCurlPageProvider.recycleBitmap( mBitmap );


			if(mBitmapB != null)
				mCurlPageProvider.recycleBitmap( mBitmapB );

			// TODO: Is there another way to create a bigger Bitmap and copy
			// original Bitmap to it more efficiently? Immutable bitmap anyone?
			mBitmap = mCurlPageProvider.createBitmapFromCache( newW, newH, bitmap.getConfig() );
			Canvas c = new Canvas(mBitmap);
			c.drawBitmap( bitmap, 0, 0, null );
			//Recycle the now unused bitmap
			mCurlPageProvider.recycleBitmap( bitmap );
			...
			if (bitmapB != null){
				...
				// TODO: Is there another way to create a bigger Bitmap and copy
				// original Bitmap to it more efficiently? Immutable bitmap anyone?
				mBitmapB= mCurlPageProvider.createBitmapFromCache( newW2, newH2, bitmapB.getConfig() );
				...
				//Recycle the now unused bitmap
				mCurlPageProvider.recycleBitmap( bitmapB );
	...
	public void swapSheet (){
		...
		mBitmap = mCurlPageProvider.createBitmapFromCache( mBitmapB.getWidth(), mBitmapB.getHeight(), mBitmapB.getConfig() );
		Canvas c = new Canvas(mBitmap);
		//Flip Matrix
		Matrix flipHorizontalMatrix = new Matrix();
		flipHorizontalMatrix.setScale( -1, 1 );
		flipHorizontalMatrix.postTranslate( mWidthAux, 0 );
		c.drawBitmap( mBitmapB, flipHorizontalMatrix, null );

		Bitmap oldSecond;
		oldSecond = mBitmapB;
		//mBitmapB = Bitmap.createBitmap( oldFirst );
		mBitmapB = mCurlPageProvider.createBitmapFromCache( oldFirst.getWidth(), oldFirst.getHeight(), oldFirst.getConfig() );

		mCurlPageProvider.recycleBitmap( oldFirst );

		Canvas c2 = new Canvas(mBitmapB);
		c2.drawBitmap( oldFirst, flipHorizontalMatrix, null);

		mCurlPageProvider.recycleBitmap( oldSecond );
	 ...
	 * Adds vertex to buffers.
	 */
	private void addHardVertex(Vertex vertex) {
		mVerticesHard.put((float) vertex.mPosX);
		mVerticesHard.put((float) vertex.mPosY);
		mVerticesHard.put((float) vertex.mPosZ);
		mColorsHard.put((float) vertex.mColor);
		mColorsHard.put((float) vertex.mColor);
		mColorsHard.put((float) vertex.mColor);
		mColorsHard.put((float) vertex.mAlpha);
		if (DRAW_TEXTURE) {
			mTexCoordsHard.put((float) vertex.mTexX);
			mTexCoordsHard.put((float) vertex.mTexY);
		}
	}
