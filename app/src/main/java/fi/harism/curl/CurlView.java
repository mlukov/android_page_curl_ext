...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	...
	// PageLogic meshes. Left and right meshes are 'static' while curl is used to
	// show page flipping.
	...
	private CurlViewIndexChangedListener mCurlViewIndexChangedListener;
	...
	/**
	 * Get current page index. PageLogic indices are zero based values presenting
				...
				if ( mAnimationTargetEvent == SET_CURL_TO_RIGHT ) {
					...
					if ( mViewMode == SHOW_ONE_PAGE || !mAllow2PagesLandscape ) {
						...
						if ( mCurlState == CURL_LEFT ) {
							...
							setIndex( mCurrentIndex - 2 );
							mPageRight.swapSheet();
						/*CAGS: Alternate method to swap sheets but inefficient in time at running,
						 * needs to get 4 images in every curl instead 2
						 *  Bitmap bitmap = mCurlPageProvider.getBitmap(mPageBitmapWidth,
								mPageBitmapHeight, mCurrentIndex);//CAGS
							Bitmap bitmap2 = mCurlPageProvider.getBitmap(mPageBitmapWidth,
					...
					if ( mViewMode == SHOW_ONE_PAGE || !mAllow2PagesLandscape ) {
						...
						/*CAGS: Alternate method to swap sheets but inefficient in time at running,
						 * needs to get 4 images in every curl instead 2
						 *	Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
								mPageBitmapHeight, mCurrentIndex+1);//CAGS
							Bitmap bitmap2 = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
								mPageBitmapHeight,  mCurrentIndex);//CAGS
	...
	public boolean onTouch(View view, MotionEvent me) {
		...
		if (mAnimate || mCurlPageProvider == null) {
			return false;
		}
		...
		try {
			...
			switch ( me.getAction() ) {
				case MotionEvent.ACTION_DOWN: {
					...
					if ( mViewMode == SHOW_TWO_PAGES ) {
						...
						// Otherwise check pointer is on right page's side.
								...
								&& mCurrentIndex < mCurlPageProvider.getPageCount() ) {
							mDragStartPos.x = rightRect.right;
							if ( !mAllowLastPageCurl
									&& mCurrentIndex >= mCurlPageProvider
									.getPageCount() - 1 ) {
								return false;
							}
				...
				case MotionEvent.ACTION_UP: {
					...
					if ( mCurlViewIndexChangedListener != null )
						mCurlViewIndexChangedListener.onCurlViewIndexChanged( mCurrentIndex );
	...
	public void setCurlPageProvider(CurlPageProvider curlPageProvider) {
		...
		mPageCurl.setCurlPageProvider( mCurlPageProvider );
		mPageLeft.setCurlPageProvider( mCurlPageProvider );
		mPageRight.setCurlPageProvider( mCurlPageProvider );
	...
	public void setIndexChangedListener( CurlViewIndexChangedListener curlViewIndexChangedListener ){

		mCurlViewIndexChangedListener = curlViewIndexChangedListener;
	}
	/**
	 * Set page index. PageLogic indices are zero based values presenting page being
	 * shown on right side of the book.
	 */
	public void setCurrentIndex(int index) {
		if (mCurlPageProvider == null || index <= 0) {
			mCurrentIndex = 0;
		} else {
			mCurrentIndex = Math.min(index,
					mCurlPageProvider.getPageCount() - 1);
		}
		updateBitmaps();
		requestRender();
	}
		...
		switch (page) {
			...
			case CURL_RIGHT: {
				...
				if (mViewMode == SHOW_ONE_PAGE || !mAllow2PagesLandscape) {
					...
					if (mCurrentIndex < mCurlPageProvider.getPageCount() - 1) {
						Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
								mPageBitmapHeight, mCurrentIndex + 1);
						mPageRight.setBitmap(bitmap,null, mCurlPageProvider.isPageHardcover( mCurrentIndex + 1 ));
					...
					if (mCurrentIndex < mCurlPageProvider.getPageCount() - 2) {
						Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
								mPageBitmapHeight, mCurrentIndex + 2);
						Bitmap bitmap2;
						if (mCurrentIndex < mCurlPageProvider.getPageCount() - 3) {
							bitmap2 = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
									mPageBitmapHeight, mCurrentIndex + 3);
						}else{
							bitmap2 = bitmap;
						}
						mPageRight.setBitmap(bitmap,bitmap2, mCurlPageProvider.isPageHardcover( mCurrentIndex + 2 ) );
			...
			case CURL_LEFT: {
				...
				if (mViewMode == SHOW_ONE_PAGE || !mAllow2PagesLandscape) {
					// If there is new/previous bitmap available get it to left page.
					if (mCurrentIndex > 1) {
						Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
								mPageBitmapHeight, mCurrentIndex - 2);
						mPageLeft.setBitmap(bitmap,null, mCurlPageProvider.isPageHardcover( mCurrentIndex - 2 ) );
						mPageLeft
								.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT));
						mPageLeft.setFlipTexture(true);
						mPageLeft.reset();
					...
					if (mCurrentIndex > 2) {
						...
						mPageLeft.setBitmap(bitmap,bitmap2, mCurlPageProvider.isPageHardcover( mCurrentIndex - 3 ));
						mPageLeft
								.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT));
						mPageLeft.setFlipTexture(false);
						mPageLeft.reset();
				...
				if (mCurrentIndex < mCurlPageProvider.getPageCount()) {
					mPageRight.setRect(mRenderer
							.getPageRect(CurlRenderer.PAGE_RIGHT));
					mPageRight.reset();
					mRenderer.addCurlMesh(mPageRight);
	...
	private void updateBitmaps() {
		if (mCurlPageProvider == null || mPageBitmapWidth <= 0
				|| mPageBitmapHeight <= 0) {
			return;
		}
		...
		if (mViewMode == SHOW_ONE_PAGE || !mAllow2PagesLandscape) {
			...
			if (rightIdx >= 0 && rightIdx < mCurlPageProvider.getPageCount()) {
				Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
						mPageBitmapHeight, rightIdx);
				mPageRight.setBitmap(bitmap,null, mCurlPageProvider.isPageHardcover( rightIdx ));
				mPageRight.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT));
				mPageRight.reset();
				mRenderer.addCurlMesh(mPageRight);
			}
			if (leftIdx >= 0 && leftIdx < mCurlPageProvider.getPageCount()) {
				Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
						mPageBitmapHeight, leftIdx);
				mPageLeft.setBitmap(bitmap,null, mCurlPageProvider.isPageHardcover( leftIdx ));
				mPageLeft.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT));
				mPageLeft.reset();
				if (mRenderLeftPage) {
					mRenderer.addCurlMesh(mPageLeft);
			...
			if (curlIdx >= 0 && curlIdx < mCurlPageProvider.getPageCount()) {
				Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
						mPageBitmapHeight, curlIdx);
				mPageCurl.setBitmap(bitmap,null, mCurlPageProvider.isPageHardcover( curlIdx ));
				if (mCurlState == CURL_RIGHT
			...
			if (rightIdx >= 0 && rightIdx < mCurlPageProvider.getPageCount()-1) {
				Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
						mPageBitmapHeight, rightIdx);
				Bitmap bitmap2 = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
						mPageBitmapHeight, rightIdx+1);
				mPageRight.setBitmap(bitmap,bitmap2, mCurlPageProvider.isPageHardcover( rightIdx+1 ));
			...
			if (leftIdx >= 0 && leftIdx < mCurlPageProvider.getPageCount()) {
				...
				mPageLeft.setBitmap(bitmap,bitmap2, mCurlPageProvider.isPageHardcover( leftIdx - 1 ));
				mPageLeft.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT));
				mPageLeft.reset();
				if (mRenderLeftPage) {
					mRenderer.addCurlMesh(mPageLeft);
			...
			if (curlIdx >= 0 && curlIdx < mCurlPageProvider.getPageCount()-1) {
				Bitmap bitmap = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
						mPageBitmapHeight, curlIdx);
				Bitmap bitmap2 = mCurlPageProvider.getPageBitmap(mPageBitmapWidth,
						mPageBitmapHeight, curlIdx+1);
				mPageCurl.setBitmap(bitmap,bitmap2, mCurlPageProvider.isPageHardcover( curlIdx ) );
				if (mCurlState == CURL_RIGHT
						|| (mCurlState == CURL_LEFT && mViewMode == SHOW_TWO_PAGES)) {
					mPageCurl.setRect(mRenderer
							.getPageRect(CurlRenderer.PAGE_RIGHT));
	...
	private void setIndex( final int currentIndex ){
	...
	/**
	 * Listener interface for handling CurlView index changes.
	 */
	public interface CurlViewIndexChangedListener {

		/**
		 * Called on every index change
		 */
		public void onCurlViewIndexChanged( final int currentIndex );
	}
